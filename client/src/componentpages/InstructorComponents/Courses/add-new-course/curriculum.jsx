...
function Curriculum() {
  ...
  const [curriculumFormData, setCurriculumFormData] = useState(
    CurriculumInitialFormData && CurriculumInitialFormData
  );
  ...
  const deleteLecture = async (itemIndex) => {
    ...
    const filteredItems = curriculumFormData.filter(
  ...
  const handleMediaBulkUpload = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        let copyCurriculumInitialData =
          areAllCourseCurriculumFormDataObjectsEmpty(curriculumFormData)
            ? []
            : [...curriculumFormData];
